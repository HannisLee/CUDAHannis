"""Activation 2:4 sparsity — Triton operator, version4.

Keeps the version3 contiguous fast path with the same block sizes (128 for
float32, 256 for float16) but removes the redundant self-value load: the lane
value is selected from the already-loaded group values instead of being
reloaded. The rank is then computed only for the current lane using lane-aware
tie breaking (a neighbour wins if its magnitude is strictly greater, or if it
ties and has a lower lane index). The masked and full-block kernels remain for
irregular shapes.
...
def activation_24_sparsity_triton_v4(x: torch.Tensor, block_groups: int = 64) -> torch.Tensor:
    """version4: contiguous fast path with lane-select value and lane-aware rank."""
    validate_input(x)
