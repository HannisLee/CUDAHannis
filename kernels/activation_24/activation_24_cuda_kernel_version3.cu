// Activation 2:4 sparsity — CUDA kernel, version3.
//
// Adds a vectorized contiguous fast path for aligned tensors. Instead of four
// scalar loads and four scalar stores per group, each group is loaded and
// stored as a single aligned transaction via GroupVec (one 16-byte load for
// float32, one 8-byte load for float16). This is the CUDA analogue of the
// Triton version3 contiguous-element fast path. The masked kernel is retained
// for irregular last-dim sizes.
...
__global__ void activation_24_sparsity_masked_kernel_v3(const scalar_t* __restrict__ x,
...
template <typename scalar_t>
__global__ void activation_24_sparsity_vec_kernel_v3(const scalar_t* __restrict__ x,
                                                     scalar_t* __restrict__ out,
...
#pragma unroll
  for (int lane = 0; lane < 4; ++lane) {
    abs_values[lane] = abs_as_float(g.d[lane]);
  }
...
#pragma unroll
    ...
    out_g.d[lane] = rank < 2 ? g.d[lane] : scalar_t(0);
  }
...
torch::Tensor activation_24_sparsity_forward_v3(torch::Tensor x) {
  ...
  if (last_dim % 4 == 0) {
    ...
    if (x.scalar_type() == torch::kFloat32) {
      activation_24_sparsity_vec_kernel_v3<float><<<blocks, threads, 0, stream>>>(
          x.data_ptr<float>(), out.data_ptr<float>(), total_groups);
    } else {
      activation_24_sparsity_vec_kernel_v3<c10::Half><<<blocks, threads, 0, stream>>>(
    ...
    const int blocks = static_cast<int>((total_groups + threads - 1) / threads);
    if (x.scalar_type() == torch::kFloat32) {
      activation_24_sparsity_masked_kernel_v3<float><<<blocks, threads, 0, stream>>>(
          x.data_ptr<float>(), out.data_ptr<float>(), rows, last_dim, groups_per_row);
    } else {
      activation_24_sparsity_masked_kernel_v3<c10::Half><<<blocks, threads, 0, stream>>>(
